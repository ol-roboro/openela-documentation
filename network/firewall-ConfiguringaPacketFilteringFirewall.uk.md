...
- Utilities to create, maintain, and display the rules that `netfilter` stores. In Enterprise Linux 8, the default firewall utility is the `firewall-cmd`, which is provided by the `firewalld` package.
...
In Enterprise Linux 8, the firewall service, `firewalld`, is enabled by default. The service is controlled by the `systemctl` command.
